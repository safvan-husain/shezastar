...
export function CartItemDetailsModal({
  isOpen,
  onClose,
  item,
  product,
            ...
            <div>
              ...
              <div className="bg-[var(--storefront-bg-subtle)] rounded-lg p-3 space-y-2 border border-[var(--storefront-border-light)]">
                {product.variants?.map((variant) => {
                  const selectedItemId = item.selectedVariantItemIds.find(
                    (id) => variant.selectedItems.some((opt) => opt.id === id),
                  );
                  const selectedItem = variant.selectedItems.find(
                      ...
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-medium text-[var(--storefront-text-primary)]">
                          {product.installationService?.availableLocations?.find(
                            (l) => l.locationId === item.installationLocationId,
                          )?.name || "Standard"}
                        </span>
                      </div>
                    )}
