    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid lg:grid-cols-[280px_1fr] gap-6">
                {/* Progress Steps - Hidden on mobile, vertical sidebar on large screens */}
                <Card className="hidden lg:block lg:sticky lg:top-6 lg:self-start">
                    <div className="flex flex-col gap-6">
                        {steps.map((s, index) => (
                            <div key={s.number} className="flex flex-col">
                                <div className="flex items-start gap-4">
                                    <div
                                        className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-lg transition-all duration-300 flex-shrink-0 ${step >= s.number
                                ...
                                {index < steps.length - 1 && (
                                    <div className="relative ml-6 my-3">
                                        <div className="w-1 h-12 bg-[var(--border)] rounded-full"></div>
                                        <div
                                            className={`absolute top-0 left-0 w-1 rounded-full transition-all duration-500 ${step > s.number ? 'h-full bg-gradient-to-b from-[var(--primary)] to-[var(--ring)]' : 'h-0'
                                                }`}
                                        ></div>
                                    </div>
                                )}
