...
import { updateSubSubCategorySeo } from '@/lib/category/category.service';
...
async function PATCHHandler(
    req: Request,
    { params }: { params: Promise<{ categoryId: string; subId: string; subSubId: string }> },
) {
    try {
        await requireAdminApiAuth({ roles: [...SEO_ADMIN_ROLES] });
        const { categoryId, subId, subSubId } = await params;
        ...
        const category = await updateSubSubCategorySeo(categoryId, subId, subSubId, parsed);
